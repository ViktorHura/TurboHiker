...
void turbohiker::World::update(const double& delta) {
    ...
    // update each entity ( entities will set their new velocities but not positions yet )
...
void turbohiker::World::handleInput(const int& key, bool keydown) {
    auto player = static_cast<turbohiker::Player *>(entities[1].get()); // second entity will always be player thus static
    if (key == 57){ // space
        //
    }else {
...
turbohiker::eType turbohiker::World::type() {
    return WorldT;
}
...
void turbohiker::World::handlePhysics(const double& delta) {
    for (auto& ep : entities ){
        ...
        if (outOfBounds(ep,newpos)){    // if that pos is out of bounds, adjust velocity
...
bool turbohiker::World::outOfBounds(std::unique_ptr<Entity> &entity, const Vector& newpos) {
...
void turbohiker::World::updateSection() {
    ...
    if (sec > section + 1 && player->position().y() < 240){ // player is in new section
        section++;
        //std::cout << "next section " << player->position().toString() << std::endl;
        newSection();
    }
...
void turbohiker::World::spawnHiker(const int& lane) {
    ...
    double posY = (section+1) * sectionSize + Random::instance()->Double(0,maxSpawnVar);
