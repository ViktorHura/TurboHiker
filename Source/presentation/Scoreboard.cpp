...
void turbohikerSFML::Scoreboard::draw(const double &delta) {
    ...
    scoreCounterText.setPosition(Transformation::instance()->posFixed(turbohiker::Utils::Vector(-4,3)));
...
turbohikerSFML::Scoreboard::Scoreboard(std::weak_ptr<sf::RenderWindow> w) {
    ...
    darken = sf::RectangleShape(turbohikerSFML::Transformation::instance()->size(turbohiker::Utils::Vector(8,6)));
...
bool compareSc(const std::tuple<int,double>& f, const std::tuple<int,double>& s){
    return std::get<1>(f) > std::get<1>(s);
}

void turbohikerSFML::Scoreboard::DrawSummary(std::shared_ptr<sf::RenderWindow> &w) {

    darken.setPosition(Transformation::instance()->posFixed(turbohiker::Utils::Vector(-4,3)));

    summaryText.setPosition(Transformation::instance()->posFixed(turbohiker::Utils::Vector(-2,2)));
    ...
    if (!ranked) {
        ...
        for (int i = 0; i < sorted.size(); i++){
            ...
            resultString += "#" + std::to_string(i + 1) + " " + intToName(std::get<0>(sorted[i])) + " : " + val + "\n";
