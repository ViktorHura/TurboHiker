...
turbohikerSFML::RacingHiker::RacingHiker(std::weak_ptr<sf::RenderWindow> w,
                                         int i)
    : turbohiker::RacingHiker(i) {
  window = std::move(w);

  // load texture and set sprite to first animation frame
  texture.loadFromFile("../Resources/PNG/enemies/competing" +
                       std::to_string(i) + ".png");
...
void turbohikerSFML::RacingHiker::shout() { sonarAnim.play(sonarSprite); }
