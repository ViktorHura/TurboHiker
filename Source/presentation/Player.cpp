...
void turbohikerSFML::Player::draw(const double &delta) {
  Transformation::instance()->setYpos(position().y() + 1); // update the camera position to player

  Transformation::instance()->setPspeed(getSpeed()); // update parallax background speed

  sprite.setPosition(Transformation::instance()->pos(position().add(
      Vector(-size().x() / 2, -size().y() * 1.5)))); // update sprite position

  playerAnim.setRate(getSpeed() / speed_normal *defaultframerate); // framerate proportional to speed
